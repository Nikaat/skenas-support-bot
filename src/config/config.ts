...
export const config = {
  ...
  admin: {
    phoneNumbers: ADMIN_PHONE_NUMBERS ? ADMIN_PHONE_NUMBERS.split(',').map((p) => p.trim()) : [],
  },
  skenas: {
...
if (config.admin.phoneNumbers.length === 0) {
  throw new Error('ADMIN_PHONE_NUMBERS is required and must contain at least one phone number');
}
